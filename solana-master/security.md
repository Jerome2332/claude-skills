...
if !context.accounts.authority.is_signer {
...
if context.accounts.token_program.key() != &spl_token::ID {
...
#[account(init, payer = payer, space = Data::DISCRIMINATOR.len() + Data::INIT_SPACE)]
pub account: Account<'info, Data>,

// Manual check if needed
if context.accounts.account.is_initialized {
...
```rust
// Anchor
if context.accounts.account_1.key() == context.accounts.account_2.key() {
...
Before shipping any program, ask:
...
9. Can arithmetic overflow/underflow be triggered?
10. Are all state transitions validated?
...
```rust
// WRONG: Using init_if_needed
#[account(init_if_needed, ...)]
pub account: Account<'info, Data>,

// WRONG: Trusting passed program ID
pub some_program: UncheckedAccount<'info>,
// then: invoke(&ix, &[some_program.to_account_info()])

// WRONG: Not checking signer
pub authority: AccountInfo<'info>,
// Missing: authority.is_signer check

// WRONG: Shared PDA without user scoping
seeds = [b"global_vault"]

// WRONG: Unchecked arithmetic
let new_balance = old_balance + amount; // Can overflow!
```
...
// CORRECT: Typed program validation
pub token_program: Program<'info, Token>,

// CORRECT: Signer type
pub authority: Signer<'info>,

// CORRECT: User-scoped PDA
...
- [Soteria Static Analyzer](https://github.com/silas-rs/soteria)
