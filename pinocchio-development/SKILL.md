...
```bash
cat ~/.claude/skills/pinocchio-development/feedback.log 2>/dev/null || echo "(no feedback log yet)"
```
...
```bash
echo "[$(date +%Y-%m-%d)] <preference in 1-2 sentences>" >> ~/.claude/skills/pinocchio-development/feedback.log
```
