...
Token-2022 requires discriminator-based validation due to variable account sizes with extensions:
...
```rust
// WRONG: transmute with unaligned data
let value: u64 = unsafe { core::mem::transmute(bytes_slice) };

// WRONG: Pointer casting to packed structs
#[repr(C, packed)]
pub struct Packed { pub a: u8, pub b: u64 }
let config = unsafe { &*(data.as_ptr() as *const Packed) };

// WRONG: Direct field access on packed structs creates unaligned references
let b_ref = &packed.b;

// WRONG: Assuming alignment without verification
...
pub fn process_batch(mut accounts: &[AccountView], mut data: &[u8]) -> ProgramResult {
    loop {
        ...
        let (instruction_accounts, instruction_data) = (
            &accounts[..account_count],
            &data[IX_HEADER_SIZE..data_offset]
        );

        process_inner_instruction(instruction_accounts, instruction_data)?;
...
**When to apply**: Only when encountering Cargo "edition2024" errors during `cargo build-sbf`.
...
See [security.md](security.md) for the comprehensive security checklist.
