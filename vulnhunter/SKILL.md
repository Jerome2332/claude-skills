...
```bash
cat ~/.claude/skills/vulnhunter/feedback.log 2>/dev/null || echo "(no feedback log yet)"
```
...
```bash
echo "[$(date +%Y-%m-%d)] <preference in 1-2 sentences>" >> ~/.claude/skills/vulnhunter/feedback.log
```
